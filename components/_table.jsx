...
import { Flex, Icon, IconButton, Input, InputGroup, InputLeftElement, Select, Spinner, Table as ChakraTable, TableContainer, Tbody, Text, Th, Thead, Tr } from '@chakra-ui/react'
...
const Table = (props) => {
	...
	const [state, dispatch] = useState({ loading: true, min: 0, max: 0, show: 0 })
	...
	const xsettings = {
		...
		searchWidth: settings && settings.searchWidth ? settings.searchWidth : 300,
	...
	useEffect(() => {
		const timer = setTimeout(() => {
			dispatch({ ...state, loading: false, min: 0, max: Number(watch('show')), show: Number(watch('show')) })
		}, 500)
	...
	return (
		<Flex direction="column" gap={6}>
			<Flex display={xsettings.search === 'off' && !select ? 'none' : 'flex'} direction={{ base: 'column', md: 'row' }} gap={6}>
				<InputGroup display={xsettings.search === 'off' ? 'none' : 'flex'} w={{ base: 'full', md: xsettings.searchWidth }}>
			...
			<TableContainer>
				<ChakraTable>
					...
					<Tbody>{!loading && fetched && results.slice(min, max).map((data, index) => td && td(data, index))}</Tbody>
				</ChakraTable>
			</TableContainer>
			...
			<Flex display={xsettings.controls === 'off' ? 'none' : 'flex'} justify="space-between" align="center" gap={6}>
				...
				<Flex align="center" gap={3}>
					...
					<IconButton variant="filled" size="lg" color="accent-1" icon={<FiChevronRight size={20} />} disabled={loading || !fetched || (results && results.length <= max)} onClick={next} />
				</Flex>
			</Flex>
		</Flex>
	)
